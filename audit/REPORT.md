...
The corrected stable bridge now has a constructor that assembles the current checked route: corrected acyclic-object realization provides the corrected closed₂ input for the bounded Verdier target structure, homotopy descent converts the checked homotopy-pullback bounded Verdier localization into the direct corrected one and reflects corrected iso-closure `Closed₂` to corrected homotopy-object `Closed₂`, WPP normalized transfer supplies finite limits and finite colimits through equalizers/coequalizers, and the derived bounded Verdier localized-equivalence input supplies the corrected preadditive, zero-object, finite-product, finite-coproduct, finite-biproduct, shift-additivity, pretriangulated, and triangulated fields. The bridge also bundles this remaining surface as `RouteSpecificInputs` and builds a `StablePackage` from that bundle. A separate `RouteSpecificLeftCalculusInputs` layer adds only the localized right-adjoint/unit input and builds `StablePackageWithLeftCalculus`, keeping corrected left calculus tied to the same route without making the stable package depend on chain-level calculus. This is still conditional, but the remaining stable-package condition surface now consists of three route-specific proof inputs rather than already-composed stable fields and no longer asks directly for corrected left calculus, the localized right-adjoint input, the direct corrected weak-equivalence localization, the homotopy-pullback bounded Verdier localization, a bounded-source closed₂ typeclass, a bounded-source strict-realization package, or an opaque WPP functor-category localization instance.
...
The invalid degreewise acyclicity definition is now isolated from the corrected construction. The project has a Lean-checked replacement predicate, checked `MetrizableLCA` bridge, checked isomorphism/shift/biproduct/finite-product closure, checked corrected bounded weak-equivalence finite-product stability, checked contractible and homotopy-equivalence bridges, a checked corrected homotopy-object `trW` surface, a checked corrected `DboundedWithCycles` localization surface, checked corrected Q/Qh comparison functors and localized-equivalence reduction, independent bounded-Verdier preadditivity, zero-object, finite-product, finite-coproduct, finite-biproduct, shift-additivity, pretriangulated, and triangulated transfers, checked finite-limit and finite-colimit reducers from equalizers and coequalizers, checked bounded-homotopy closed₂, and a checked corrected Metrizable stable package constructor from route-specific inputs. The next formalization step is discharging those route-specific inputs, especially the corrected acyclic-object cone/distinguished-triangle realization, homotopy-equivalence descent for corrected acyclicity, and the WPP normalized finite-shape transfer, without relying on legacy `boundedExactWeakEquivalence`.
