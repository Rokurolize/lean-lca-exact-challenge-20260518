...
That localized-equivalence reduction now also supplies an independent corrected preadditivity route. `DboundedWithCycles.preadditiveOfBoundedVerdierLocalizationInput` transports preadditivity back from the bounded corrected homotopy Verdier quotient along the induced equivalence using mathlib's fully-faithful preadditive transfer; the metrizable bridge exposes this as `BoundedDerivedWithCycles.Metrizable.preadditiveOfBoundedVerdierLocalizationInput`. This is still conditional on the bounded-source closed₂, corrected acyclic closed₂, and composite-localization inputs, but it no longer routes this field through chain-level left calculus.
