...
The corrected stable bridge now has a constructor that assembles the current checked route: bounded-source realization plus corrected acyclic closed₂ provide the bounded Verdier target structure, homotopy descent plus the localized right-adjoint input supplies corrected left calculus, WPP transfer supplies finite limits and finite colimits through equalizers/coequalizers, and the bounded Verdier localized-equivalence input supplies the corrected pretriangulated and triangulated fields. The bridge also bundles this remaining surface as `RouteSpecificInputs` and builds a `StablePackage` from that bundle. This is still conditional, but the remaining condition surface now consists of route-specific proof inputs rather than already-composed stable fields.
