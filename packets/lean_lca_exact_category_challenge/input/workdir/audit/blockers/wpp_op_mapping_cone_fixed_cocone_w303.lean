...
/-!
W303: universal-property boundary for the fixed WPP-op mapping-cone cocone.

W302 fixed the W276 object-level mapping-cone comparison to a concrete diagram
`mappingConeDiagram` and a concrete cocone `mappingConeCocone`.  This file follows
the useful signal from support W281 and reduces the remaining colimit proof for
that fixed cocone to explicit `desc`/`fac`/`uniq` data.
...
/-- Machine-readable frontier state for W303. -/
