...
- exact categoryのconflation chainから作るローカル`YonedaExt`型。degree 0はHom、正次数はconflation chainを生成元とする自由加法可換群を、extension鎖の同型、hom等式、分裂一段拡大、Baer sum witnessから来る生成関係で割った群としてコンパイルする。分裂一段拡大が0になり、Baer sum witnessがある場合に`sum = e₁ + e₂`になることもLean定理として確認している
