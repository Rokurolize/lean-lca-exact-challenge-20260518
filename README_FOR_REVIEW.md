...
一方、標準的なYoneda Extとして必要な構成のうち、左側の商関係全体にも降りる双線形なYoneda積、degree 0のhead/tail作用を商群上の写像として降ろす証明、任意長のhom head/tailを含む長さ方向の関係を完全に扱う部分は未完成である。また`Dbounded`はordinary categoryのlocalizationとしては構成したが、stable infinity-categoryそのもののモデルではない。このgoalでは、これらを残したまま`update_goal(status="complete")`を呼んではならない。
