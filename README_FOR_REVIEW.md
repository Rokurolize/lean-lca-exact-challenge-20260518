...
一方、標準的なYoneda Extとして必要な商関係のうち、任意の一段拡大対に対してBaer sum witnessを構成する部分、長さの異なるYoneda積の関係を商として実装する部分は未完成である。また`Dbounded`はordinary categoryのlocalizationとしては構成したが、stable infinity-categoryそのもののモデルではない。このgoalでは、これらを残したまま`update_goal(status="complete")`を呼んではならない。
