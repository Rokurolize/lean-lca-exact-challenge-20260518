...
- `MetrizableLCA`の一段拡大2つからcoordinatewise product extensionを作り、Baer sum witnessの`productExtension`部分に必要な射影互換性を与えるLean定理
