...
- exact categoryのconflation chainから作るローカル`YonedaExt`型。degree 0はHom、正次数はconflation chainを生成元とする自由加法可換群を、termwiseなextension同型、hom等式、分裂一段拡大を零元にする生成関係で割った群としてコンパイルする
...
一方、標準的なYoneda Extとして必要な商関係のうち、任意の鎖同型、Baer sumを表すpushout/pullback図式、長さの異なるYoneda積の関係を商として実装する部分は未完成である。また`Dbounded`はordinary categoryのlocalizationとしては構成したが、stable infinity-categoryそのもののモデルではない。このgoalでは、これらを残したまま`update_goal(status="complete")`を呼んではならない。
