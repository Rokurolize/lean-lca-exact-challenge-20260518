...
/-- Identify the actual enriched mapping object out of the cone with the realized strict
path-fiber pullback. -/
def directDGMappingConePathFiberEnrichedHomIso (T : ComplexCategory)
    {K L : ComplexCategory} (f : K ⟶ L) :
    (directDGMappingConeObject f ⟶[SSet] directDGObject T) ≅
      dgMappingSSetRealizationFunctor.obj (dgHomPrecompositionPathFiber T f) :=
  eqToIso (directDG_enrichedHom_eq (dgMappingConeObject f) T) ≪≋
    dgMappingConePathFiberSSetIso T f

/-- First leg of the enriched cone homotopy-fiber square. -/
def directDGMappingConePathFiberFst (T : ComplexCategory)
    {K L : ComplexCategory} (f : K ⟶ L) :
    (directDGMappingConeObject f ⟶[SSet] directDGObject T) ⟶
      dgMappingSSetRealizationFunctor.obj (dgHomZModuleCochainComplex L T) :=
  (directDGMappingConePathFiberEnrichedHomIso T f).hom ≫
    dgMappingSSetRealizationFunctor.map
      (Limits.pullback.fst (dgHomPrecompositionZeroEndpoints T f)
        (dgHomPrecompositionPathEndpoints T K))

/-- Path leg of the enriched cone homotopy-fiber square. -/
def directDGMappingConePathFiberSnd (T : ComplexCategory)
    {K L : ComplexCategory} (f : K ⟶ L) :
    (directDGMappingConeObject f ⟶[SSet] directDGObject T) ⟶
      dgMappingSSetRealizationFunctor.obj (dgHomPrecompositionPathObject T K) :=
  (directDGMappingConePathFiberEnrichedHomIso T f).hom ≫
    dgMappingSSetRealizationFunctor.map
      (Limits.pullback.snd (dgHomPrecompositionZeroEndpoints T f)
        (dgHomPrecompositionPathEndpoints T K))

/-- The actual enriched cone mapping object is the strict simplicial pullback of the realized
path-object endpoint diagram, hence a path-object model of the homotopy fiber. -/
theorem directDGMappingCone_enrichedHom_isPathFiberPullback (T : ComplexCategory)
    {K L : ComplexCategory} (f : K ⟶ L) :
    IsPullback
      (directDGMappingConePathFiberFst T f)
      (directDGMappingConePathFiberSnd T f)
      (dgMappingSSetRealizationFunctor.map (dgHomPrecompositionZeroEndpoints T f))
      (dgMappingSSetRealizationFunctor.map (dgHomPrecompositionPathEndpoints T K)) := by
  apply (dgHomPrecompositionPathFiberSSet_isPullback T f).of_iso'
...
/-- A degree `-1` cochain acts covariantly on dg Hom complexes by postcomposition. -/
def dgHomPostcompositionHomotopyCochain (T : ComplexCategory)
    {X Y : ComplexCategory}
    (z : CochainComplex.HomComplex.Cochain X.obj Y.obj (-1)) :
    CochainComplex.HomComplex.Cochain
      (dgHomZModuleCochainComplex T X) (dgHomZModuleCochainComplex T Y) (-1) where
  v p q hpq := ModuleCat.ofHom
    { toFun := fun γ ↦ γ.comp z (by omega)
      map_add' := by
        intro γ γ'
        rw [CochainComplex.HomComplex.Cochain.add_comp]
      map_smul' := by
        intro r γ
        rw [CochainComplex.HomComplex.Cochain.smul_comp] }

theorem dgHomPostcompositionHomotopyCochain_delta
    (T : ComplexCategory) {X Y : ComplexCategory}
    (g₀ g₁ : X ⟶ Y)
    (z : CochainComplex.HomComplex.Cochain X.obj Y.obj (-1))
    (hz : CochainComplex.HomComplex.δ (-1) 0 z =
      CochainComplex.HomComplex.Cochain.ofHom g₀.hom -
        CochainComplex.HomComplex.Cochain.ofHom g₁.hom) :
    CochainComplex.HomComplex.δ (-1) 0
        (dgHomPostcompositionHomotopyCochain T z) =
      CochainComplex.HomComplex.Cochain.ofHom (dgHomZModulePostcomposition T g₀) -
        CochainComplex.HomComplex.Cochain.ofHom (dgHomZModulePostcomposition T g₁) := by
  ext p
  apply ModuleCat.hom_ext
  apply LinearMap.ext
  intro γ
  simp only [CochainComplex.HomComplex.δ_zero_cochain_v,
    dgHomPostcompositionHomotopyCochain,
    CochainComplex.HomComplex.Cochain.ofHom_v,
    CochainComplex.HomComplex.Cochain.sub_v,
    dgHomZModuleCochainComplex, dgHomZModulePostcomposition,
    ConcreteCategory.comp_apply]
  rw [CochainComplex.HomComplex.δ_comp γ z (by omega) (p + 1) 0 p
    (by omega) rfl (neg_add_cancel 1)]
  rw [hz]
  simp only [CochainComplex.HomComplex.Cochain.sub_comp,
    CochainComplex.HomComplex.Cochain.ofHom_comp,
    Int.negOnePow_neg, Int.negOnePow_one, Units.neg_smul, one_smul]
  abel

/-- Chain homotopies act covariantly on dg Hom cochain complexes. -/
def dgHomZModulePostcompositionHomotopy (T : ComplexCategory)
    {X Y : ComplexCategory} {g₀ g₁ : X ⟶ Y}
    (h : Homotopy g₀.hom g₁.hom) :
    Homotopy (dgHomZModulePostcomposition T g₀)
      (dgHomZModulePostcomposition T g₁) :=
  (CochainComplex.HomComplex.Cochain.equivHomotopy _ _).symm
    ⟨dgHomPostcompositionHomotopyCochain T
        (CochainComplex.HomComplex.Cochain.ofHomotopy h), by
      apply dgHomPostcompositionHomotopyCochain_delta
      exact CochainComplex.HomComplex.Cochain.δ_ofHomotopy h⟩

@[reassoc]
theorem dgHomZModulePostcomposition_comp (T : ComplexCategory)
    {X Y Z : ComplexCategory} (f : X ⟶ Y) (g : Y ⟶ Z) :
    dgHomZModulePostcomposition T f ≫ dgHomZModulePostcomposition T g =
      dgHomZModulePostcomposition T (f ≫ g) := by
  ext n γ
  change (γ.comp (CochainComplex.HomComplex.Cochain.ofHom f.hom) (add_zero n)).comp
      (CochainComplex.HomComplex.Cochain.ofHom g.hom) (add_zero n) =
    γ.comp (CochainComplex.HomComplex.Cochain.ofHom (f.hom ≫ g.hom)) (add_zero n)
  rw [CochainComplex.HomComplex.Cochain.ofHom_comp,
    CochainComplex.HomComplex.Cochain.comp_assoc]

/-- Covariant dg Hom image of the cone triangle nullhomotopy. -/
def dgMappingConePostcompositionCompositeNullhomotopy (T : ComplexCategory)
    {K L : ComplexCategory} (f : K ⟶ L) :
    Homotopy
      (dgHomZModulePostcomposition T f ≫
        dgHomZModulePostcomposition T (dgMappingConeInr f)) 0 :=
  (Homotopy.ofEq (dgHomZModulePostcomposition_comp T f (dgMappingConeInr f))).trans
    (by
      simpa only [dgHomZModulePostcomposition, map_zero] using
        dgHomZModulePostcompositionHomotopy T (dgMappingConeTriangleHomotopy f))

/-- The covariant cone nullhomotopy as a map into the maps-in path object. -/
def dgMappingConeHomFromToPathObject (T : ComplexCategory)
    {K L : ComplexCategory} (f : K ⟶ L) :
    dgHomZModuleCochainComplex T K ⟶ dgHomPostcompositionPathObject T f :=
  HomologicalComplex.pathObject.lift
    (dgHomZModulePostcomposition T f ≫
      dgHomZModulePostcomposition T (dgMappingConeInr f)) 0
    (dgMappingConePostcompositionCompositeNullhomotopy T f)

theorem dgMappingConeHomFrom_pathFiber_condition (T : ComplexCategory)
    {K L : ComplexCategory} (f : K ⟶ L) :
    dgHomZModulePostcomposition T f ≫ dgHomPostcompositionZeroEndpoints T f =
      dgMappingConeHomFromToPathObject T f ≫ dgHomPostcompositionPathEndpoints T f := by
  apply Limits.prod.hom_ext
  · simp [dgHomPostcompositionZeroEndpoints, dgHomPostcompositionPathEndpoints,
      dgMappingConeHomFromToPathObject]
  · simp [dgHomPostcompositionZeroEndpoints, dgHomPostcompositionPathEndpoints,
      dgMappingConeHomFromToPathObject]

/-- Canonical map from maps into `K` to the strict path fiber of maps into `L → Cone(f)`. -/
def dgMappingConeHomFromToPathFiber (T : ComplexCategory)
    {K L : ComplexCategory} (f : K ⟶ L) :
    dgHomZModuleCochainComplex T K ⟶ dgHomPostcompositionPathFiber T f :=
  Limits.pullback.lift
    (dgHomZModulePostcomposition T f)
    (dgMappingConeHomFromToPathObject T f)
    (dgMappingConeHomFrom_pathFiber_condition T f)

/-- Homotopy extracted from the maps-in path-fiber coordinate. -/
def dgHomPostcompositionPathFiberHomotopy (T : ComplexCategory)
    {K L : ComplexCategory} (f : K ⟶ L) :
    Homotopy
      (Limits.pullback.fst (dgHomPostcompositionZeroEndpoints T f)
          (dgHomPostcompositionPathEndpoints T f) ≫
        dgHomZModulePostcomposition T (dgMappingConeInr f)) 0 := by
  let R := dgHomPostcompositionPathFiber T f
  let a : R ⟶ dgHomZModuleCochainComplex T L := Limits.pullback.fst _ _
  let p : R ⟶ dgHomPostcompositionPathObject T f := Limits.pullback.snd _ _
  have hcondition := Limits.pullback.condition
    (dgHomPostcompositionZeroEndpoints T f) (dgHomPostcompositionPathEndpoints T f)
  have h₀ : a ≫ dgHomZModulePostcomposition T (dgMappingConeInr f) =
      p ≫ HomologicalComplex.pathObject.π₀
        (dgHomZModuleCochainComplex T (dgMappingConeObject f)) := by
    simpa [a, p, dgHomPostcompositionZeroEndpoints, dgHomPostcompositionPathEndpoints,
      Category.assoc] using congrArg (fun q ↦ q ≫ Limits.prod.fst) hcondition
  have h₁ : p ≫ HomologicalComplex.pathObject.π₁
      (dgHomZModuleCochainComplex T (dgMappingConeObject f)) = 0 := by
    simpa [a, p, dgHomPostcompositionZeroEndpoints, dgHomPostcompositionPathEndpoints,
      Category.assoc] using congrArg (fun q ↦ q ≫ Limits.prod.snd) hcondition.symm
  exact (Homotopy.ofEq h₀).trans
    ((HomologicalComplex.pathObject.homotopy₀₁
      (dgHomZModuleCochainComplex T (dgMappingConeObject f))
      (fun i ↦ ⟨i + 1, rfl⟩)).compLeft p) |>.trans (Homotopy.ofEq h₁)

/-- Degreewise inverse from the maps-in path fiber to `Hom(T,K)`. -/
def dgHomPostcompositionPathFiberToKernelDegree (T : ComplexCategory)
    {K L : ComplexCategory} (f : K ⟶ L) (n : ℤ) :
    (dgHomPostcompositionPathFiber T f).X n ⟶
      (dgHomZModuleCochainComplex T K).X n :=
  let z := CochainComplex.HomComplex.Cochain.ofHomotopy
    (dgHomPostcompositionPathFiberHomotopy T f)
  ModuleCat.ofHom
    { toFun := fun x ↦ (z.v n (n - 1) (by omega) x).comp
        (CochainComplex.mappingCone.fst
          ((boundedCochainComplex MetrizableLCA.{0}).ι.map f)).1 (by omega)
      map_add' := by
        intro x y
        simp
      map_smul' := by
...
theorem dgHomPostcompositionPathFiberToKernelDegree_comm (T : ComplexCategory)
    {K L : ComplexCategory} (f : K ⟶ L) (n m : ℤ)
    (hnm : (ComplexShape.up ℤ).Rel n m) :
    dgHomPostcompositionPathFiberToKernelDegree T f n ≫
        (dgHomZModuleCochainComplex T K).d n m =
      (dgHomPostcompositionPathFiber T f).d n m ≫
        dgHomPostcompositionPathFiberToKernelDegree T f m := by
  change n + 1 = m at hnm
  subst m
  apply ModuleCat.hom_ext
  apply LinearMap.ext
  ...
  let z := CochainComplex.HomComplex.Cochain.ofHomotopy
    (dgHomPostcompositionPathFiberHomotopy T f)
  let a := Limits.pullback.fst (dgHomPostcompositionZeroEndpoints T f)
    (dgHomPostcompositionPathEndpoints T f)
  change CochainComplex.HomComplex.δ n (n + 1)
      ((z.v n (n - 1) (by omega) x).comp
        (CochainComplex.mappingCone.fst
          ((boundedCochainComplex MetrizableLCA.{0}).ι.map f)).1 (by omega)) =
    (z.v (n + 1) n (by omega)
      ((dgHomPostcompositionPathFiber T f).d n (n + 1) x)).comp
        (CochainComplex.mappingCone.fst
          ((boundedCochainComplex MetrizableLCA.{0}).ι.map f)).1 (by omega)
  rw [CochainComplex.HomComplex.δ_comp]
  simp only [CochainComplex.HomComplex.Cocycle.δ_eq_zero,
    CochainComplex.HomComplex.Cochain.comp_zero, add_zero]
  have hz := CochainComplex.HomComplex.Cochain.congr_v
    (CochainComplex.HomComplex.Cochain.δ_ofHomotopy
      (dgHomPostcompositionPathFiberHomotopy T f)) n n (add_zero n)
  have hz' := ConcreteCategory.congr_hom hz x
  simp only [CochainComplex.HomComplex.Cochain.ofHom_v,
    ConcreteCategory.comp_apply, zero_apply, sub_zero] at hz'
  rw [hz']
  change ((CochainComplex.HomComplex.Cochain.ofHom (dgMappingConeInr f).hom).comp
      (a.f n x) (zero_add n)).comp
      (CochainComplex.mappingCone.fst
        ((boundedCochainComplex MetrizableLCA.{0}).ι.map f)).1 (by omega) = _
  rw [CochainComplex.HomComplex.Cochain.comp_assoc,
    CochainComplex.mappingCone.inr_fst,
    CochainComplex.HomComplex.Cochain.comp_zero]

/-- Inverse chain map from the maps-in path fiber to `Hom(T,K)`. -/
def dgHomPostcompositionPathFiberToKernel (T : ComplexCategory)
    {K L : ComplexCategory} (f : K ⟶ L) :
    dgHomPostcompositionPathFiber T f ⟶ dgHomZModuleCochainComplex T K where
  f n := dgHomPostcompositionPathFiberToKernelDegree T f n
  comm' n m h := dgHomPostcompositionPathFiberToKernelDegree_comm T f n m h

theorem dgMappingConeHomFrom_pathFiber_roundtrip (T : ComplexCategory)
    {K L : ComplexCategory} (f : K ⟶ L) :
    dgMappingConeHomFromToPathFiber T f ≫
        dgHomPostcompositionPathFiberToKernel T f = 𝟙 _ := by
  ext n γ
  simp [dgMappingConeHomFromToPathFiber,
    dgHomPostcompositionPathFiberToKernel,
    dgHomPostcompositionPathFiberToKernelDegree,
    dgMappingConeHomFromToPathObject]

theorem dgHomPostcompositionPathFiber_kernel_roundtrip (T : ComplexCategory)
    {K L : ComplexCategory} (f : K ⟶ L) :
    dgHomPostcompositionPathFiberToKernel T f ≫
        dgMappingConeHomFromToPathFiber T f = 𝟙 _ := by
  apply Limits.pullback.hom_ext
  · ext n x
    simp [dgMappingConeHomFromToPathFiber,
      dgHomPostcompositionPathFiberToKernel,
...
/-- Maps into `K` are exactly the strict path-object homotopy fiber of maps into
`L → Cone(f)`. -/
def dgMappingConeHomFromPathFiberIso (T : ComplexCategory)
    {K L : ComplexCategory} (f : K ⟶ L) :
    dgHomZModuleCochainComplex T K ≅ dgHomPostcompositionPathFiber T f where
  hom := dgMappingConeHomFromToPathFiber T f
