...
theorem alternatingAlexanderWhitney_right_unit_component
    (X : SimplicialObject (QuotientModule)) (n q : ℕ)
    (hq : n + q = n) :
  ...
  change ((alternatingChains X).X n ◁ 𝟙 (𝟙_ (QuotientModule))) ≫ _ = _
...
theorem alternatingAlexanderWhitney_left_unitality
    ...
    (λ_ (alternatingChains X)).inv =
      (alternatingFaceMapComplex (QuotientModule)).map (λ_ X).inv ≫
...
theorem alternatingAlexanderWhitney_right_unitality
    ...
    (ρ_ (alternatingChains X)).inv =
      (alternatingFaceMapComplex (QuotientModule)).map (ρ_ X).inv ≫
    ...
    (λ_ (quotientModuleDoldKanEquivalence.functor.obj X)).inv =
      quotientModuleDoldKanEquivalence.functor.map (λ_ X).inv ≫
        ...
        (normalizedAlexanderWhitneyUnit ▷
          quotientModuleDoldKanEquivalence.functor.obj X) := by
  symm
  change
    (normalizedMooreComplex (QuotientModule)).map (λ_ X).inv ≫
        normalizedAlexanderWhitney
          (𝟙_ (SimplicialObject (QuotientModule))) X ≫
        (normalizedAlexanderWhitneyUnit ▷
          (normalizedMooreComplex (QuotientModule)).obj X) =
      (λ_ ((normalizedMooreComplex (QuotientModule)).obj X)).inv
  simp only [normalizedAlexanderWhitney]
  rw [← Category.assoc]
  rw [← Category.assoc]
  have hinc :=
    (inclusionOfMooreComplex (QuotientModule)).naturality (λ_ X).inv
  change
    (normalizedMooreComplex (QuotientModule)).map (λ_ X).inv ≫
        inclusionOfMooreComplexMap
          ((𝟙_ (SimplicialObject (QuotientModule))) ⊗ X) =
      inclusionOfMooreComplexMap X ≫
        (alternatingFaceMapComplex (QuotientModule)).map (λ_ X).inv at hinc
  rw [hinc]
  have htensor :
    ...
    change
      HomologicalComplex.tensorHom
          ...
          HomologicalComplex.tensorHom normalizedAlexanderWhitneyUnit
            (𝟙 ((normalizedMooreComplex (QuotientModule)).obj X)) = _
    calc
      _ = HomologicalComplex.tensorHom
          (DoldKan.PInftyToNormalizedMooreComplex
            ...
            normalizedAlexanderWhitneyUnit)
          (DoldKan.PInftyToNormalizedMooreComplex X ≫
            𝟙 ((normalizedMooreComplex (QuotientModule)).obj X)) := by
...
theorem normalizedAlexanderWhitney_right_unitality
  ...
  symm
  change
    (normalizedMooreComplex (QuotientModule)).map (ρ_ X).inv ≫
        normalizedAlexanderWhitney X
          (𝟙_ (SimplicialObject (QuotientModule))) ≫
        ((normalizedMooreComplex (QuotientModule)).obj X ◁
          normalizedAlexanderWhitneyUnit) =
      (ρ_ ((normalizedMooreComplex (QuotientModule)).obj X)).inv
  simp only [normalizedAlexanderWhitney]
  rw [← Category.assoc]
  rw [← Category.assoc]
  have hinc :=
    (inclusionOfMooreComplex (QuotientModule)).naturality (ρ_ X).inv
  change
    (normalizedMooreComplex (QuotientModule)).map (ρ_ X).inv ≫
        inclusionOfMooreComplexMap
          (X ⊗ (𝟙_ (SimplicialObject (QuotientModule)))) =
      inclusionOfMooreComplexMap X ≫
        (alternatingFaceMapComplex (QuotientModule)).map (ρ_ X).inv at hinc
  rw [hinc]
  have htensor :
      HomologicalComplex.tensorHom
            (DoldKan.PInftyToNormalizedMooreComplex X)
    ...
    change
      HomologicalComplex.tensorHom
            ...
            (DoldKan.PInftyToNormalizedMooreComplex
              (𝟙_ (SimplicialObject (QuotientModule)))) ≫
          HomologicalComplex.tensorHom
            (𝟙 ((normalizedMooreComplex (QuotientModule)).obj X))
    ...
    _ = _ := by
      ...
      have hwhisker :
        ...
        (MonoidalCategory.comp_whiskerRight
          (inclusionOfMooreComplexMap X)
          (DoldKan.PInftyToNormalizedMooreComplex X)
          (𝟙_ (ChainComplex (QuotientModule) ℕ))).symm
      rw [hwhisker]
      have hsplit := (DoldKan.splitMonoInclusionOfMooreComplexMap X).id
      change inclusionOfMooreComplexMap X ≫
        DoldKan.PInftyToNormalizedMooreComplex X = 𝟙 _ at hsplit
