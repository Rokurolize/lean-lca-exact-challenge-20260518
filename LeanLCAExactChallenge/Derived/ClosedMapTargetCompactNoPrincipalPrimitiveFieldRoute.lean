...
/-!
W929 feeds the W928 closed-map target-compact-outer-nonbottom-primitive-field
route while removing the principal-universe support premise from the target
compactness primitive.
-/
...
/-- W929 primitive form of target-codomain compactness without the principal-univ premise. -/
abbrev targetCodomainCompactNoPrincipalPrimitiveW929
...
/-- W929 assembles W928's target-codomain compactness outer-nonbottom primitive. -/
abbrev targetCodomainCompactOuterNonbottomPrimitiveOfNoPrincipalPrimitiveW929
    (targetCodomainCompactSpaceProvider :
      ∀ Y : WalkingParallelPairᵒᵖ ⥤ MetrizableLCA.{0},
        targetCodomainCompactNoPrincipalPrimitiveW929 Y) :
    ∀ (Y : WalkingParallelPairᵒᵖ ⥤ MetrizableLCA.{0}),
      targetCodomainCompactOuterNonbottomPrimitiveW928 Y := by
  intro Y f hfne _hfprincipal
  exact targetCodomainCompactSpaceProvider Y (f := f) hfne

/--
W929 replaces W928's target-codomain compactness primitive with the same
non-bottom filter condition after dropping the vacuous `principal univ` premise.
-/
structure MetrizableWppClosedMapTargetCompactNoPrincipalPrimitiveFieldLeavesW929 :
    Type 2 where
  rightOpenBoundary :
...
/-- W929 projects W928 leaves from the no-principal primitive. -/
def w928LeavesOfTargetCompactNoPrincipalPrimitiveFieldsW929
    (leaves :
  ...
  targetCodomainCompactSpaceProvider :=
    targetCodomainCompactOuterNonbottomPrimitiveOfNoPrincipalPrimitiveW929
...
/-- W929 W733 concrete leaves after projecting W928 leaves. -/
def concreteLeavesOfClosedMapTargetCompactNoPrincipalPrimitiveFieldsW929
    (leaves :
      MetrizableWppClosedMapTargetCompactNoPrincipalPrimitiveFieldLeavesW929) :
...
/-- W929 W732 route data after projecting W928 leaves. -/
def routeDataProviderOfClosedMapTargetCompactNoPrincipalPrimitiveFieldsW929
    (leaves :
      MetrizableWppClosedMapTargetCompactNoPrincipalPrimitiveFieldLeavesW929) :
...
theorem directBoundedLeftCalculusOfClosedMapTargetCompactNoPrincipalPrimitiveFieldsW929
    (leaves :
      MetrizableWppClosedMapTargetCompactNoPrincipalPrimitiveFieldLeavesW929) :
...
/-- W929 exposes W928's target compactness without the principal-univ premise. -/
def closedMapTargetCompactNoPrincipalPrimitiveFieldsInputNamesW929 :
    List String :=
  ["raw right-open closed-quotient-cover WPP-limit boundary proposition",
    ...
    "primitive target-codomain compactness filter and meet nonbottom\
...
theorem closedMapTargetCompactNoPrincipalPrimitiveFieldsInputNamesW929_count :
    closedMapTargetCompactNoPrincipalPrimitiveFieldsInputNamesW929.length =
      39 :=
  rfl

/-- Current checked W929 state. -/
structure MetrizableClosedMapTargetCompactNoPrincipalPrimitiveFieldsRouteStateW929 :
    Type where
  seed : String
  declarations : List String
  targetCompactNoPrincipalPrimitiveFieldResult : String
  exactAtResult : String
  refinedInputs : List String
  remainingInputs : List String
  productSuccessClaimed : Bool

def currentMetrizableClosedMapTargetCompactNoPrincipalPrimitiveFieldsRouteStateW929 :
    MetrizableClosedMapTargetCompactNoPrincipalPrimitiveFieldsRouteStateW929
    where
  seed :=
    "w929-closed-map-target-compact-no-principal-primitive-field-\
      target-compact-outer-nonbottom-primitive-field"
  declarations :=
    ["MetrizableWppClosedMapTargetCompactNoPrincipalPrimitiveFieldLeavesW929",
      "targetCodomainCompactNoPrincipalPrimitiveW929",
      "targetCodomainCompactOuterNonbottomPrimitiveOfNoPrincipalPrimitiveW929",
      "w928LeavesOfTargetCompactNoPrincipalPrimitiveFieldsW929",
      ...
      "boundedDerivedInfinityCategoryOfClosedMapTargetCompactNoPrincipalPrimitiveFieldsW929",
      "closedMapTargetCompactNoPrincipalPrimitiveFieldsInputNamesW929",
      "closedMapTargetCompactNoPrincipalPrimitiveFieldsInputNamesW929_count"]
  targetCompactNoPrincipalPrimitiveFieldResult :=
    "proved: primitive target-codomain compactness without the principal-univ\
      premise supplies W928's outer-nonbottom field"
  exactAtResult :=
    "proved: closed-map target-compact-no-principal-primitive-field\
      target-compact-outer-nonbottom-primitive-field\
      ...
      component-fields mapped-cokernel-field fields feed the W928 stable\
      ExactAt route"
  refinedInputs :=
    ["primitive target-codomain compactness without principal-univ premise"]
  remainingInputs :=
    closedMapTargetCompactNoPrincipalPrimitiveFieldsInputNamesW929
  productSuccessClaimed := false

theorem currentW929ClosedMapTargetCompactNoPrincipalPrimitiveFieldsRoute_productSuccess :
    (let state :=
        currentMetrizableClosedMapTargetCompactNoPrincipalPrimitiveFieldsRouteStateW929;
