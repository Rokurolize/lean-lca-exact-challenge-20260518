...
/-!
W927 feeds the W926 closed-map target-compact-cluster-primitive-field route
while exposing the inner `Filter.NeBot` witness as non-bottom inequality.
-/
...
/-- W927 primitive form of target-codomain compactness without inner `NeBot`. -/
abbrev targetCodomainCompactNonbottomPrimitiveW927
...
/-- W927 assembles W926's target-codomain compactness cluster primitive. -/
abbrev targetCodomainCompactClusterPrimitiveOfNonbottomPrimitiveW927
    (targetCodomainCompactSpaceProvider :
      ∀ Y : WalkingParallelPairᵒᵖ ⥤ MetrizableLCA.{0},
        targetCodomainCompactNonbottomPrimitiveW927 Y) :
    ∀ (Y : WalkingParallelPairᵒᵖ ⥤ MetrizableLCA.{0}),
      targetCodomainCompactClusterPrimitiveW926 Y := by
  intro Y f _ hf
  obtain ⟨x, hx, hne⟩ := targetCodomainCompactSpaceProvider Y (f := f) hf
  exact ⟨x, hx, ⟨hne⟩⟩

/--
W927 replaces W926's target-codomain compactness primitive with the
non-bottom inequality hidden by the inner `Filter.NeBot` class.
-/
structure MetrizableWppClosedMapTargetCompactNonbottomPrimitiveFieldLeavesW927 :
    Type 2 where
  rightOpenBoundary :
...
/-- W927 projects W926 leaves from the target-compactness nonbottom primitive. -/
def w926LeavesOfTargetCompactNonbottomPrimitiveFieldsW927
    (leaves :
  ...
  targetCodomainCompactSpaceProvider :=
    targetCodomainCompactClusterPrimitiveOfNonbottomPrimitiveW927
...
/-- W927 W733 concrete leaves after projecting W926 leaves. -/
def concreteLeavesOfClosedMapTargetCompactNonbottomPrimitiveFieldsW927
    (leaves :
      MetrizableWppClosedMapTargetCompactNonbottomPrimitiveFieldLeavesW927) :
...
/-- W927 W732 route data after projecting W926 leaves. -/
def routeDataProviderOfClosedMapTargetCompactNonbottomPrimitiveFieldsW927
    (leaves :
      MetrizableWppClosedMapTargetCompactNonbottomPrimitiveFieldLeavesW927) :
...
theorem directBoundedLeftCalculusOfClosedMapTargetCompactNonbottomPrimitiveFieldsW927
    (leaves :
      MetrizableWppClosedMapTargetCompactNonbottomPrimitiveFieldLeavesW927) :
...
/-- W927 exposes W926's target compactness without inner `NeBot`. -/
def closedMapTargetCompactNonbottomPrimitiveFieldsInputNamesW927 :
    List String :=
  ["raw right-open closed-quotient-cover WPP-limit boundary proposition",
    ...
    "primitive target-codomain compactness neighborhood-filter meet nonbottom\
...
theorem closedMapTargetCompactNonbottomPrimitiveFieldsInputNamesW927_count :
    closedMapTargetCompactNonbottomPrimitiveFieldsInputNamesW927.length =
      39 :=
  rfl

/-- Current checked W927 state. -/
structure MetrizableClosedMapTargetCompactNonbottomPrimitiveFieldsRouteStateW927 :
    Type where
  seed : String
  declarations : List String
  targetCompactNonbottomPrimitiveFieldResult : String
  exactAtResult : String
  refinedInputs : List String
  remainingInputs : List String
  productSuccessClaimed : Bool

def currentMetrizableClosedMapTargetCompactNonbottomPrimitiveFieldsRouteStateW927 :
    MetrizableClosedMapTargetCompactNonbottomPrimitiveFieldsRouteStateW927
    where
  seed :=
    "w927-closed-map-target-compact-nonbottom-primitive-field-\
      target-compact-cluster-primitive-field"
  declarations :=
    ["MetrizableWppClosedMapTargetCompactNonbottomPrimitiveFieldLeavesW927",
      "targetCodomainCompactNonbottomPrimitiveW927",
      "targetCodomainCompactClusterPrimitiveOfNonbottomPrimitiveW927",
      "w926LeavesOfTargetCompactNonbottomPrimitiveFieldsW927",
      ...
      "boundedDerivedInfinityCategoryOfClosedMapTargetCompactNonbottomPrimitiveFieldsW927",
      "closedMapTargetCompactNonbottomPrimitiveFieldsInputNamesW927",
      "closedMapTargetCompactNonbottomPrimitiveFieldsInputNamesW927_count"]
  targetCompactNonbottomPrimitiveFieldResult :=
    "proved: primitive target-codomain compactness neighborhood-filter meet\
      nonbottom inequality supplies W926's inner Filter.NeBot field"
  exactAtResult :=
    "proved: closed-map target-compact-nonbottom-primitive-field\
      target-compact-cluster-primitive-field\
      ...
      component-fields mapped-cokernel-field fields feed the W926 stable\
      ExactAt route"
  refinedInputs :=
    ["primitive target-codomain compactness neighborhood-filter meet nonbottom"]
  remainingInputs :=
...
theorem currentW927ClosedMapTargetCompactNonbottomPrimitiveFieldsRoute_productSuccess :
    (let state :=
        currentMetrizableClosedMapTargetCompactNonbottomPrimitiveFieldsRouteStateW927;
