...
/-!
W924 feeds the W923 closed-map target-surjective-primitive-field route while
exposing the target-codomain `CompactSpace` leaf as compactness of the
universe.
-/
...
/-- W924 primitive form of the target-codomain compactness field. -/
abbrev targetCodomainCompactPrimitiveW924
    (Y : WalkingParallelPairᵒᵖ ⥤ MetrizableLCA.{0}) : Prop :=
  IsCompact (Set.univ : Set (wppOpCodomainW441 Y))

/-- W924 assembles the target-codomain compactness field. -/
abbrev targetCodomainCompactSpaceOfPrimitiveW924
    (targetCodomainCompactSpaceProvider :
      ∀ Y : WalkingParallelPairᵒᵖ ⥤ MetrizableLCA.{0},
        targetCodomainCompactPrimitiveW924 Y) :
    ∀ (Y : WalkingParallelPairᵒᵖ ⥤ MetrizableLCA.{0}),
      CompactSpace (wppOpCodomainW441 Y) := by
  intro Y
  exact
    { isCompact_univ := by
        simpa [targetCodomainCompactPrimitiveW924] using
          targetCodomainCompactSpaceProvider Y }

/--
W924 replaces W923's target-codomain `CompactSpace` field with the primitive
compactness proposition hidden by that class.
-/
structure MetrizableWppClosedMapTargetCompactPrimitiveFieldLeavesW924 :
    Type 2 where
  rightOpenBoundary :
...
/-- W924 projects W923 leaves from the target-compactness primitive field. -/
def w923LeavesOfTargetCompactPrimitiveFieldsW924
    (leaves :
  ...
  targetCodomainCompactSpaceProvider :=
    targetCodomainCompactSpaceOfPrimitiveW924
...
/-- W924 W733 concrete leaves after projecting W923 leaves. -/
def concreteLeavesOfClosedMapTargetCompactPrimitiveFieldsW924
...
/-- W924 W732 route data after projecting W923 leaves. -/
def routeDataProviderOfClosedMapTargetCompactPrimitiveFieldsW924
    (leaves : MetrizableWppClosedMapTargetCompactPrimitiveFieldLeavesW924) :
    MetrizableWppBoundaryRelationTargetClosednessNormalizedDataProviderW732 :=
  routeDataProviderOfClosedMapTargetSurjectivePrimitiveFieldsW923
    (w923LeavesOfTargetCompactPrimitiveFieldsW924 leaves)

theorem directBoundedLeftCalculusOfClosedMapTargetCompactPrimitiveFieldsW924
    (leaves : MetrizableWppClosedMapTargetCompactPrimitiveFieldLeavesW924) :
...
/-- W924 exposes W923's target-codomain compactness field as primitive. -/
def closedMapTargetCompactPrimitiveFieldsInputNamesW924 :
    List String :=
  ["raw right-open closed-quotient-cover WPP-limit boundary proposition",
    ...
    "primitive target-codomain compactness of the universe for every target diagram",
...
theorem closedMapTargetCompactPrimitiveFieldsInputNamesW924_count :
    closedMapTargetCompactPrimitiveFieldsInputNamesW924.length =
      39 :=
  rfl

/-- Current checked W924 state. -/
structure MetrizableClosedMapTargetCompactPrimitiveFieldsRouteStateW924 :
    Type where
  seed : String
  declarations : List String
  targetCompactPrimitiveFieldResult : String
  exactAtResult : String
  refinedInputs : List String
  remainingInputs : List String
  productSuccessClaimed : Bool

def currentMetrizableClosedMapTargetCompactPrimitiveFieldsRouteStateW924 :
    MetrizableClosedMapTargetCompactPrimitiveFieldsRouteStateW924
    where
  seed :=
    "w924-closed-map-target-compact-primitive-field-\
      target-surjective-primitive-field"
  declarations :=
    ["MetrizableWppClosedMapTargetCompactPrimitiveFieldLeavesW924",
      "targetCodomainCompactPrimitiveW924",
      "targetCodomainCompactSpaceOfPrimitiveW924",
      "w923LeavesOfTargetCompactPrimitiveFieldsW924",
  ...
  targetCompactPrimitiveFieldResult :=
    "proved: primitive target-codomain compactness of the universe supplies\
      W923's CompactSpace field"
  exactAtResult :=
    "proved: closed-map target-compact-primitive-field\
      target-surjective-primitive-field\
      ...
      component-fields mapped-cokernel-field fields feed the W923 stable\
      ExactAt route"
  refinedInputs :=
    ["primitive target-codomain compactness of the universe"]
  remainingInputs :=
    closedMapTargetCompactPrimitiveFieldsInputNamesW924
  productSuccessClaimed := false

theorem currentW924ClosedMapTargetCompactPrimitiveFieldsRoute_productSuccess :
    (let state :=
        currentMetrizableClosedMapTargetCompactPrimitiveFieldsRouteStateW924;
