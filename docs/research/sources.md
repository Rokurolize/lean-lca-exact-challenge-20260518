...
- ローカルmathlib `Mathlib/CategoryTheory/Localization/Triangulated.lean`: ordinary pretriangulated/triangulated structureをlocalizationへ移すAPIを確認した。v83では同型閉包側の`IsTriangulatedClosed₂`を仮定して、`ExactAcyclicHomotopyVerdierCategory`のpretriangulated/triangulated構造を取り出すために使った。これはstable infinity-categoryそのものではない。
- ローカルmathlib `Mathlib/Algebra/Homology/HomotopyCategory/Triangulated.lean`: additive category上のcochain complexのhomotopy categoryがtriangulatedになる経路を確認した。この段階の`Dbounded`はまだこのhomotopy-category経路へ無条件には接続していない。
- ローカルmathlib `Mathlib/CategoryTheory/Triangulated/Subcategory.lean`: `ObjectProperty.trW`と、object propertyがtriangulated subcategoryである場合に`trW.HasLeftCalculusOfFractions`が得られるAPIを確認し、v79からv83の条件付きVerdier局所化frontierに使った。v84ではbounded homotopy objectをhomotopy category内のfull subcategoryとして明示する背景にも使った。
- ローカルmathlib `Mathlib/CategoryTheory/MorphismProperty/Basic.lean`: `MorphismProperty.inverseImage`と`inverseImage_iff`を確認し、homotopy category側の`trW`をbounded complex側へ引き戻すために使った。
- ローカルmathlib `Mathlib/CategoryTheory/Localization/LocalizerMorphism.lean`: `LocalizerMorphism`、`LocalizerMorphism.ofEq`、`localizedFunctor`、`LocalizerMorphism.IsLocalizedEquivalence.of_equivalence`を確認し、direct bounded exact localizationからhomotopy/Verdier pullback localizationへの比較functorと、同型閉性仮定下のlocalized equivalenceを作るために使った。
- ローカルmathlib `Mathlib/CategoryTheory/Comma/Arrow/Basic.lean`: `Arrow.isoOfNatIso`を確認し、shift後の射とfunctorで送ったshift射の同型比較に使った。
- ローカルmathlib `Mathlib/CategoryTheory/Shift/Localization.lean`: morphism propertyのshift compatibilityとlocalized functorの`CommShift`を確認し、homotopy/Verdier pullback弱同値類にもshift構造を移すために使った。
...
- `LeanLCAExactChallenge/Derived/Bounded.lean`: bounded cochain complexのfull subcategoryを、mapping coneがexact acyclicである射でlocalizeする`Dbounded`をコンパイル確認した。さらに`BoundedDerivedInfinityCategory`を`Dbounded`のnerveが定める`SSet.QCat`として公開し、`Dbounded.infinityNerve_quasicategory`と`Dbounded.homotopyCategoryIso`でquasicategory性とhomotopy category比較を確認した。v76では`BoundedComplexCategory`、boundednessの同型閉性・shift閉性、exact acyclicityの同型不変性を確認した。v77ではexact acyclicityとbounded exact weak equivalenceのshift互換性を確認した。v78ではbounded complex側のzero objectと、left calculus of fractions仮定下の`Dbounded`のpreadditive/zero/additive shift APIを確認した。v79ではexact acyclic complexesをhomotopy category上のobject property `exactAcyclicHomotopyObject`として切り出し、bounded exact weak equivalenceがその`trW`のinverse imageへ入ること、同型閉性仮定下の一致、triangulated subcategory仮定下の`trW` left calculus of fractionsを確認した。v80ではzero complexのexact acyclicity、`exactAcyclicHomotopyObject`のzero object包含、同型閉性を仮定したshift安定性、同型閉性と`IsTriangulatedClosed₂`からの`IsTriangulated`、その条件下での`trW` left calculus of fractionsを確認した。v81では`exactAcyclicHomotopyIsoClosure`、同型閉包のshift安定性、`trW`不変性、同型閉包側の`IsTriangulatedClosed₂`から元の`trW` left calculusを得る条件付きAPIを確認した。v82では`boundedHomotopyExactWeakEquivalence`、direct弱同値からhomotopy/Verdier pullback弱同値への包含、同型閉性仮定下の一致、shift互換性、`BoundedHomotopyDerivedCategory`、`BoundedHomotopyDerivedQuasicategory`、`Dbounded.homotopyComparison`を確認した。v83では同型閉包側の`IsTriangulatedClosed₂`を仮定した`ExactAcyclicHomotopyVerdierCategory`、`ExactAcyclicHomotopyVerdierQuasicategory`、left calculus、triangulation compatibility、preadditive/zero/shift/additive shift/pretriangulated/triangulated構造を確認した。v84ではbounded cochain complexのmapping cone閉性、homotopy category上の`boundedHomotopyObject`、`BoundedHomotopyCategory`、zero object、shift閉性、bounded objectのmapping cone閉性を確認した。v85では`boundedExactWeakEquivalenceToHomotopyExactWeakEquivalenceOfIsoClosed`、そのlocalized equivalence instance、`Dbounded.homotopyComparisonEquivalenceOfIsoClosed`により、exact acyclic homotopy objectの同型閉性仮定下で`Dbounded`とhomotopy/Verdier pullback localizationが普通の圏として同値になることを確認した。
