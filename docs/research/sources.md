...
- `LeanLCAExactChallenge/Ext/Yoneda.lean`: exact categoryのconflation chainから作るローカル`YonedaExt`をコンパイル確認した。degree 0はHom、正次数はextension chainを生成元とする自由加法可換群を、termwiseなextension同型、hom等式、分裂一段拡大を零元にする生成関係で割った群である。
