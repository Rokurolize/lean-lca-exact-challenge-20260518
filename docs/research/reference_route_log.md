...
- 上の補題を`ShortExactExtension`側へ接続し、MetrizableLCAの一段拡大2つからproduct extensionを作るAPIを追加した。`ShortExactExtension.BaerSumData`が求めるproduct段階の射影互換性は、このAPIで取り出せる。
