...
このログは、Leanコードそのものでは見えにくい調査経路を読める形で残すためのものだ。今回の成果物は、strict metrizable LCA列がQuillen exact categoryをなすところまではローカルLean証明で到達し、さらに形式的なYoneda extension chain群とbounded complexのlocalizationまで進めた。そのうえで、標準的なYoneda Extとbounded derived infinity-categoryとして何が残るかを明示する記録である。
...
有効な近道は三つあった。第一に、`ShortComplex`が既にあり、conflationの入れ物を自作せずに済んだ。第二に、`FreeAbelianGroup`があり、extension chainを生成元とする加法可換群を短く実装できた。第三に、`CochainComplex.mappingCone`と`MorphismProperty.Localization`があり、bounded complexのlocalizationを一から商圏として作らずに済んだ。

ただし、これらだけではproduct theoremは完成しない。strict metrizable LCA sequenceのpushout/pullback安定性はローカル証明で閉じ、形式的なYoneda extension chain群とordinary localizationとしての`Dbounded`まではコンパイルした。しかし標準的なYoneda Extの商関係とBaer sum図式、stable infinity-categoryとしてのenhancementは未解決のままである。
