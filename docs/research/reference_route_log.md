...
ただし、これらだけではproduct theoremは完成しない。strict metrizable LCA sequenceのpushout/pullback安定性はローカル証明で閉じ、Yoneda extension chainの局所的な商群、一段左spliceと純粋な正次数左spliceの商への下降、split factorを含む鎖の0関係、一段拡大2つのproduct extensionとchosen binary biproduct端点への移送、ordinary localizationとしての`Dbounded`まではコンパイルした。しかし標準的なYoneda Extで必要な対角pullbackと余対角pushoutによる任意Baer sum witnessの完成、左側の商関係にも降りる双線形Yoneda積、stable infinity-categoryとしてのenhancementは未解決のままである。
