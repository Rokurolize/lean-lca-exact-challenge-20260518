...
- Extとderived categoryの宣言は、strict LCA exact categoryから使えるローカル構成に置き換えた。左端一段拡大のBaer和を固定tailの前へ貼る場合の加法等式までは証明済みである。ただし、標準的なYoneda Extに必要な左側の商関係全体にも降りる双線形積、degree 0やhom尾部を含む長さ方向の関係、stable infinity-categoryとしての構造は未完成である。
