...
- 一段拡大2つのcoordinatewise product extension、明示的な積モデルからmathlibのchosen binary biproduct端点への移送、Baer sum witnessの`productExtension`部分に必要な射影互換性はLeanで証明済みである。
...
`terminal_outcome/terminal_outcome.json`は`product_complete=false`を保つ必要がある。後続のproduct-success goalでは、chosen binary biproduct端点へ移送済みのproduct extensionを対角pullbackと余対角pushoutに通すBaer sum witness構成、左側の商関係にも降りる双線形Yoneda積、degree 0やhom尾部を含む積の互換性、また`D^b(C,E)`をordinary localization以上の意味で扱うためのinfinity-category側のモデルを実装する必要がある。
