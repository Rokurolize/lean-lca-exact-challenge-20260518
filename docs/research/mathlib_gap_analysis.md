...
- Extとderived categoryの宣言は、strict LCA exact categoryから使えるローカル構成に置き換えた。左端一段拡大の同型関係とBaer和を固定tailの前へ貼る場合の等式・加法等式、および一段拡大の後ろにdegree 0のhom tailが付く一段ケースをpushout拡大へ落とす等式までは証明済みである。ただし、標準的なYoneda Extに必要な左側の商関係全体にも降りる双線形積、degree 0のpullback側の作用、任意長のhom tailを含む長さ方向の関係、stable infinity-categoryとしての構造は未完成である。
...
`terminal_outcome/terminal_outcome.json`は`product_complete=false`を保つ必要がある。後続のproduct-success goalでは、左側の商関係全体にも降りる双線形Yoneda積、degree 0のpullback側の作用、任意長のhom tailを含む積の互換性、また`D^b(C,E)`をordinary localization以上の意味で扱うためのinfinity-category側のモデルを実装する必要がある。
