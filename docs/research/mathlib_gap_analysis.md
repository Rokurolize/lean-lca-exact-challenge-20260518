...
- Extとderived categoryの宣言は、strict LCA exact categoryから使えるローカル構成に置き換えた。ただし、標準的なYoneda Extに必要な左側の商関係にも降りる双線形積、degree 0やhom尾部を含む長さ方向の関係、stable infinity-categoryとしての構造は未完成である。
...
`terminal_outcome/terminal_outcome.json`は`product_complete=false`を保つ必要がある。後続のproduct-success goalでは、左側の商関係にも降りる双線形Yoneda積、degree 0やhom尾部を含む積の互換性、また`D^b(C,E)`をordinary localization以上の意味で扱うためのinfinity-category側のモデルを実装する必要がある。
