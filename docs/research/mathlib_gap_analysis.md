...
`terminal_outcome/terminal_outcome.json`は`product_complete=false`を保つ必要がある。後続のproduct-success goalでは、左側の商関係全体にも降りる双線形Yoneda積、degree 0のhead作用を商群上の写像として降ろす証明、任意長のhom head/tailを含む積の互換性、また`D^b(C,E)`をordinary localization以上の意味で扱うためのinfinity-category側のモデルを実装する必要がある。
